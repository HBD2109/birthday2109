...
$('document').ready(function(){
	...
	// Floating Balloon Animation Functions
	function loopOne() {
	...
	// AUTOMATED SEQUENCE (Runs with 3-second pauses between actions)
	setTimeout(function step1_TurnOnLights() {
		...
		setTimeout(function step2_PlayMusic() {
			$('#play').fadeOut('slow');
			var audio = $('.song')[0];
			if (audio) { audio.play(); }
			...
			setTimeout(function step3_Banner() {
				$('#bannar_coming').fadeOut('slow');
				$('.bannar').addClass('bannar-come');

				setTimeout(function step4_Balloons() {
					...
					setTimeout(function step5_Cake() {
						$('#cake_fadein').fadeOut('slow');
						$('.cake').fadeIn('slow');

						setTimeout(function step6_Candle() {
							$('#light_candle').fadeOut('slow');
							$('.fuego').fadeIn('slow');

							setTimeout(function step7_WishMessage() {
								...
								// Centered balloon positioning for 6-letter name (POONAM)
								$('#b11').animate({top: 240, left: vw - 250}, 500);
								$('#b22').animate({top: 240, left: vw - 150}, 500);
								$('#b33').animate({top: 240, left: vw - 50}, 500);
								$('#b44').animate({top: 240, left: vw + 50}, 500);
								...
								setTimeout(function step8_Story() {
									$('#story').fadeOut('slow');
